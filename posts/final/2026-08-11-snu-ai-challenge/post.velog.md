# 구조를 넣고, 실패를 기록했다: SNU AI Challenge 참여 회고

## 대회 개요

[SNU AI Challenge](https://snuaichallenge.github.io/)는 서울대학교 데이터사이언스대학원이 자체 가공한 데이터를 공개하고, 국내외 대학 학부생이 최신 AI 모델을 직접 개발해 성능을 겨룰 수 있도록 마련한 경진대회다.

2026년 대회의 주제는 **텍스트로 풀어보는 장면의 재구성**이었다. 스토리라인을 설명하는 문장과 순서가 섞인 이미지 프레임 네 장을 함께 보고, 문장의 맥락에 맞게 이미지의 시간 순서를 복원해야 했다. 단일 이미지 인식을 넘어 여러 장면의 관계와 시간적 전개를 이해하는 멀티모달 추론 능력을 평가하는 과제였다.

예선은 온라인 경진대회로 진행됐고, 예선 기간의 public leaderboard는 전체 test data 중 70%를 사용했다. 이 글에서 언급하는 public score도 이 leaderboard의 Exact Match Accuracy를 뜻한다. 나는 2026년 7월 9일부터 25일까지 멀티모달 모델 설계와 학습·추론 실험을 진행했다.

## 먼저 결과부터

문장과 뒤섞인 이미지 네 장의 시간 순서를 맞히는 대회에 참여했다. 정답 순열을 한 번에 고르게 하는 대신 이미지 사이의 선후 관계와 각 이미지의 위치를 나눠 학습하면서 성능을 끌어올렸고, 실험 과정의 최고 public leaderboard Exact Match는 **0.86585**였다. 그보다 더 크게 남은 것은 좋은 아이디어를 내는 법보다 아이디어를 제대로 비교하는 법에 대한 교훈이었다.

## 네 장의 사진, 순서는 하나다

[SNU AI Challenge](https://snuaichallenge.github.io/)의 과제는 설명만 들으면 단순해 보인다. 사건을 설명하는 문장 하나와 시간 순서가 섞인 이미지 네 장이 주어지고, 각 이미지가 원래 몇 번째였는지 맞히면 된다.
...
이미지 한 장만 잘 이해해서는 이 문제를 풀기 어려웠다.

1. 각 이미지의 상태와 행동을 읽어야 한다.
2. 두 이미지 중 무엇이 먼저인지 판단해야 한다.
3. 문장에 담긴 사건의 흐름과 이미지 네 장을 하나의 순서로 연결해야 한다.

나는 이 문제를 **Language-guided Visual Temporal Ordering**으로 정의했다. 이름을 붙이고 나니 찾아봐야 할 대상도 선명해졌다. 이미지 분류보다 temporal ordering과 procedural reasoning에 가까운 선행 연구를 찾아보기 시작했고, 모델을 평가할 기준도 단순한 이미지 이해 능력에서 관계와 순서를 얼마나 잘 다루는지로 바뀌었다.

## 점수보다 먼저 실행 조건을 맞췄다

대회 모델은 인터넷이 없는 환경에서 단일 RTX 3090 24GB에 올라가야 했고, 전체 테스트 추론을 24시간 안에 끝내야 했다. 외부 데이터, 상용 API, 앙상블에도 제약이 있었다.

그래서 초반에 대회 규정과 실행 조건을 Codex skill로 구조화했다. 모델이나 새로운 전처리 아이디어를 검토할 때마다 공개 시점, 외부 자원 사용 여부, VRAM, 추론 시간, 오프라인 재현 가능성을 같은 기준으로 확인하기 위해서였다.

이 과정에서 대회의 목표는 accuracy 하나가 아님을 배웠다.
...
## Backbone은 같은 조건에서 비교했다

처음에는 temporal reasoning에 특화된 모델이 가장 유리할 것이라고 생각했다. 하지만 이름이나 공개 benchmark만 보고 결정하지 않고, 세 backbone을 같은 대회 train QLoRA 조건에서 비교했다.
...
특정 temporal ordering task를 위해 만든 checkpoint는 아니지만, 범용 시각·언어 이해와 reasoning capacity가 대회 데이터에 더 잘 적응할 수 있다고 보고 비교에 포함했다.

예상과 다른 결과가 나왔다. Temporal·procedural reasoning에 맞춰 post-training된 TPRU가 대회 분포에서도 가장 강할 것이라는 보장은 없었다. 세 모델 중에는 Qwen3.5-9B가 가장 높았고, 이후 실험의 backbone으로 선택했다.

중요한 건 zero-shot 결과를 섞지 않고 비교했다는 사실이다. 모두 같은 데이터와 비슷한 학습 예산으로 대회 입출력 형식에 적응시킨 뒤 비교했다. 완벽한 비교는 아니었지만, 적어도 모델 이름과 크기 외의 조건을 가능한 한 맞추려 했다.

## 문제의 구조를 모델에 넣자 점수가 올랐다

이미지 네 장이 만들 수 있는 순서는 `4! = 24`개다. 가장 단순한 방법은 24개 class 중 하나를 바로 고르게 하는 것이다. 하지만 이 방식은 오답 사이의 거리를 가르쳐 주지 못한다.
...
최종 설정에서는 Pairwise와 Position을 중심으로 학습했다. 추론할 때는 두 head가 내놓은 확률을 이용해 24개 유효 순열을 모두 채점하고, 가장 일관된 하나를 선택했다.
...
Token Attention은 기존 mean pooling을 버리지 않고, 문장을 참고하는 attention residual을 더했다. 사진 전체를 평균내는 동시에 문장과 관련된 작은 영역에 확대경을 한 번 더 대는 방식이었다. 점수는 올랐지만, token 수 증가와 attention pooling, head 재학습이 한꺼번에 들어갔다는 한계가 있었다. 어느 요소가 얼마나 기여했는지는 분리하지 못했다.
...
Event Boundary도 기대만큼 움직이지 않았다. Backbone이 이미 시간 표현을 충분히 반영했을 수도 있고, 규칙으로 나눈 event slot이 실제 사건 구조와 맞지 않았을 수도 있다. 한 번의 실험만으로는 원인을 확정하기 어려웠다.
...
학습 weight를 그대로 둔 추론 전략만으로도 MultiHead 기준 **0.84816에서 0.85689**로 올랐다. 모든 sample에 무조건 계산을 더 쓰지 않고, 모델의 판단이 갈린 경우에만 재검증한 점도 대회 제약 안에서 의미가 있었다.
...
Frontier 이후에는 더 그럴듯해 보이는 가설을 여러 개 시험했다.
...
가장 크게 떨어진 것은 모든 구성요소를 한꺼번에 공동 학습한 Frontier-2였다. 좋은 모듈을 한 objective에 넣었다고 각 장점이 자동으로 보존되지는 않았다.

Caption 실험도 흥미로웠다. 저해상도 이미지에서 놓치는 정보를 문장이 보완할 것이라고 예상했지만, 생성된 caption은 원본 이미지를 언어로 압축한 정보였다. 부정확한 설명이 섞이자 보완 정보보다 noise에 가까워졌다. 입력 정보의 양이 늘었다고 유효한 정보까지 늘어나는 것은 아니었다.

RL에서는 24개뿐인 action 중 sample마다 5개를 중복 허용해 뽑았다. 균등한 정책을 가정하면 서로 다른 action은 평균 약 4.6개만 관측하고, 정답 action이 한 번이라도 포함될 확률도 약 19.2%에 그친다. 대회가 끝난 뒤 돌아보니 24개 action을 전부 평가해 expected reward를 계산하는 편이 sampling noise를 줄이는 설계로 더 자연스러웠다.
...
물론 4B, 9B, 27B 세 점만으로 모델을 키우면 점수가 선형으로 오른다고 말할 수는 없다. 이번 대회에서 내가 관측한 범위에서는 task-specific 구조만큼 backbone capacity도 강한 병목이었다. 초반에 architecture 아이디어를 깊게 파기 전에 family와 scale을 작은 비용으로 넓게 훑었어야 했다.

## 대회가 끝난 뒤 더 선명하게 보인 실수
...
특히 zero-init residual은 학습 시작 시 영향력이 0이다. 충분한 update를 받기 전에 학습을 끝냈다면 “효과가 없다”가 아니라 “1 epoch 안에는 효과가 나타나지 않았다”가 더 정확한 결론이다. 실험 비용이 부족해도 결론의 강도까지 과하게 높여서는 안 됐다.
...
Task-specific adapter를 먼저 깊게 파고 27B는 뒤늦게 시험했다. 다음에는 `model family × scale` matrix를 짧게 screening한 뒤 구조 실험으로 들어갈 것이다. 좁고 깊은 탐색을 시작하기 전에 어디가 가장 큰 축인지부터 확인해야 한다.
