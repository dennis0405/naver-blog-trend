---
working_title: "구조를 넣고, 실패를 기록했다: SNU AI Challenge 참여 회고"
post_type: "project_retrospective"
target_reader: "멀티모달 AI 대회에서 모델과 실험을 어떻게 설계하는지 궁금한 개발자·연구자"
target_queries:
  - "SNU AI Challenge 후기"
  - "멀티모달 AI 대회 회고"
  - "이미지 시간 순서 예측"
project_context:
  project_name: "SNU AI Challenge"
  my_role: "멀티모달 모델 설계와 학습·추론 실험"
  period: "2026.07.09-2026.07.25"
privacy_level: "public_sanitized"
must_include:
  - "대회 문제와 실행 제약"
  - "Backbone 비교와 MultiHead 설계"
  - "성공한 실험과 실패한 실험"
  - "최종 결과"
  - "실험 운영에서 배운 점"
must_not_include:
  - "비공개 저장소와 서버 정보"
  - "개인 API token과 credential"
  - "팀원의 개인 정보"
  - "공개 여부가 불분명한 대회 데이터"
status: "draft"
---
...
## 네 장의 사진을 시간순으로 놓는 문제

[SNU AI Challenge](https://snuaichallenge.github.io/)의 과제는 설명만 보면 단순하다. 사건을 설명하는 문장 하나와 시간 순서가 섞인 이미지 네 장이 주어지고, 각 이미지가 원래 몇 번째였는지 맞히면 된다.
...
이 문제를 풀려면 이미지 한 장만 잘 이해해서는 부족했다.

1. 각 이미지의 상태와 행동을 읽어야 한다.
2. 두 이미지 중 무엇이 먼저인지 판단해야 한다.
3. 문장에 담긴 사건의 흐름과 이미지 네 장을 하나의 순서로 연결해야 한다.

나는 이 문제를 **Language-guided Visual Temporal Ordering**으로 정의했다. 이름을 붙이니 무엇을 찾아봐야 하는지도 선명해졌다. 이미지 분류보다 temporal ordering과 procedural reasoning에 가까운 선행 연구를 찾아보기 시작했고, 모델을 평가할 기준도 단순한 이미지 이해 능력에서 관계와 순서를 얼마나 잘 다루는지로 바뀌었다.

## 점수만 높으면 제출할 수 있는 것은 아니었다
...
이 과정에서 대회의 목표가 accuracy 하나가 아니라는 점을 배웠다.
...
| Backbone | 구성 | Public score |
|---|---|---:|
| TPRU-7B | TPRU-7B + 대회 train QLoRA | 0.46945 |
| InternVL2.5-8B-MPO | InternVL + 대회 train QLoRA | 0.69982 |
| Qwen3.5-9B | NF4 4-bit + QLoRA | **0.77661** |

결과는 예상과 달랐다. Temporal·procedural reasoning에 맞춰 post-training된 TPRU가 대회 분포에서도 가장 강할 것이라는 보장은 없었다. 세 모델 중에는 Qwen3.5-9B가 가장 높았고, 이후 실험의 backbone으로 선택했다.

여기서 중요했던 것은 zero-shot 결과를 섞어 비교하지 않았다는 점이다. 모두 같은 데이터와 비슷한 학습 예산으로 대회 입출력 형식에 적응시킨 뒤 비교했다. 완벽한 비교는 아니었지만, 적어도 모델 이름과 크기 외의 조건을 가능한 한 맞추려 했다.

## 가장 큰 첫 개선은 문제의 구조를 모델에 넣는 것이었다
...
Token Attention은 기존 mean pooling을 버리지 않고, 문장을 참고하는 attention residual을 더했다. 사진 전체를 평균내는 동시에 문장과 관련된 작은 영역에 확대경을 한 번 더 대는 방식이었다. 이 실험은 점수를 올렸지만, token 수 증가와 attention pooling, head 재학습이 한꺼번에 들어갔다는 한계가 있었다. 어느 요소가 얼마나 기여했는지는 분리하지 못했다.
...
Event Boundary도 기대만큼 움직이지 않았다. Backbone이 이미 시간 표현을 충분히 반영했을 수도 있고, 규칙으로 나눈 event slot이 실제 사건 구조와 맞지 않았을 수도 있다. 한 번의 실험만으로 원인을 확정할 수는 없었다.
...
학습 weight를 바꾸지 않은 이 추론 전략만으로도 MultiHead 기준 **0.84816에서 0.85689**로 올랐다. 모든 sample에 무조건 계산을 더 쓰지 않고, 모델의 판단이 갈린 경우에만 재검증한 점도 대회 제약 안에서 의미가 있었다.
...
물론 4B, 9B, 27B 세 점만으로 모델을 키우면 점수가 선형으로 오른다고 말할 수는 없다. 다만 이번 대회에서 내가 관측한 범위에서는 task-specific 구조만큼 backbone capacity도 강한 병목이었다. 초반에 architecture 아이디어를 깊게 파기 전에 family와 scale을 작은 비용으로 넓게 훑었어야 했다.
...
특히 zero-init residual은 학습 시작 시 영향력이 0이다. 충분한 update를 받기 전에 학습을 끝냈다면 “효과가 없다”가 아니라 “1 epoch 안에는 효과가 나타나지 않았다”가 더 정확한 결론이다. 실험 비용이 부족하다는 이유로 결론의 강도까지 과하게 높여서는 안 됐다.
